...
if [[ "$UNAME" == "Darwin" ]]; then
  DART_LIB=$DART_SRC/runtime/xcodebuild/${DART_MODE}$DART_ARCH
  DART_GEN=$DART_SRC/runtime/xcodebuild/DerivedSources/${DART_MODE}$DART_ARCH
  LIBRARY_GROUP_START= # not needed - mac linker 'just works' with circular dependencies
  LIBRARY_GROUP_END=
  WEB_GEN="$DART_LIB/../dart-runtime.build/${DART_MODE}$DART_ARCH/dart.build/Objects-normal/*/web_gen.o"
else
  if [[ "$UNAME" != "Linux" ]]; then
...
LTFLAGS="--tag=CC" $APXS -S CC=g++ -c $COPTS -o mod_dart.so -Wc,-Wall -Wc,-Werror -I $DART_SRC/runtime -lstdc++ -I $DART_GEN \
-Wl,-Wl$LIBRARY_GROUP_START,$DART_LIB/libdart_export.a,$DART_LIB/libdart_builtin.a,$DART_LIB/libdart_lib_withcore.a,$DART_LIB/libdart_vm.a,$DART_LIB/libjscre.a,$DART_LIB/libdouble_conversion.a,$WEB_GEN$LIBRARY_GROUP_END \
